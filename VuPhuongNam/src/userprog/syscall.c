...
int fd_num = 2;            /*The number of fd, starting from 2, only increase without decrease*/
/* file descriptor */
struct fd{
    int num;
    struct file *file;
...
bool char_pointer_valid(char *pointer)
{
...
int read(int num,void *buffer,unsigned size)
{
...
int write(int num,const void* buffer,unsigned size)
{
...
void seek(int num, unsigned position)
{
  struct fd *fd = find_fd_by_num(num);
  if(fd == NULL)
  {
